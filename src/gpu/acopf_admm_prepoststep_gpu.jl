...
function acopf_admm_poststep(
    ...
    info.user.err_pg, info.user.err_qg = check_generator_bounds(mod, u_curr)
    ...
    info.user.num_rateA_viols, info.user.err_rateA = check_linelimit_violation(data, v_curr)
