...
function acopf_admm_poststep(
    ...
    info.objval = sum(data.generators[g].coeff[data.generators[g].n-2]*(mod.baseMVA*sol.v_curr[mod.gen_start+2*(g-1)])^2 +
                      data.generators[g].coeff[data.generators[g].n-1]*(mod.baseMVA*sol.v_curr[mod.gen_start+2*(g-1)]) +
